...
uint8_t buffer[512];
uint8_t token;
SD_readSingleBlock(0, buffer, &token);
...
if(bpb->jumpBoot[0]!=0xE9 && bpb->jumpBoot[0]!=0xEB)   //check if it is boot sector
{
  ...
  SD_readSingleBlock(partition->firstSector, buffer, &token);//read the bpb sector
  bpb = (struct BS_Structure *)buffer;
...
while(retry <10)
{ if(!SD_readSingleBlock(FATEntrySector, buffer, &token)) break; retry++;}
